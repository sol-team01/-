...
<body>
	<div id="warp">
		...
		<div id="main">
			...
			<!-- 			내가 한 거 -->
			<div>
				<h3 style="font-size: 22px;">${read.b_title}</h3>
				<%-- 					<legend>글제목: :${read.b_title}</legend> --%>
				<div
					style="display: flex; align-items: center; margin-bottom: 14px;">
					<div>${read.i_name}</div>
					<div style="margin-left: 20px; font-size: 16px;">${read.b_datetime}</div>
					<div>${read.b_hit}</div>
				</div>
				<div style="height:280px; border:1px solid red;margin-bottom:14px;">${read.b_text}</div>
				<%-- 								<label for="id">작성자: </label> ${read.i_name} <br> <label --%>
				<%-- 									for="content">내용: </label> ${read.b_text} <br> <label --%>
				<%-- 									for="date">작성일: </label> ${read.b_datetime} <br> <label --%>
				<%-- 									for="hit">조회수: </label> ${read.b_hit} --%>
				...
				<c:choose>
					...
					<div style="display:flex; align-items:center; margin-left:auto; width:fit-content;">
						...
						<form action="${cp}/board/modifyBoard">
							<input type="hidden" name="B_no" value="${read.b_no}"> <input
								type="submit" value="글 수정" class="writeBoardBtnK">
						</form>
						<form action="${cp}/board/del" style="margin-left:10px;">
							<input type="hidden" name="B_no" value="${read.b_no}"> <input
								type="submit" value="글 삭제" class="writeBoardBtnKBorder">
						</form>
					</div>
					</c:when>
					<c:otherwise>
				...
				<hr>
				<span id="reply_area"> <c:forEach var="replyList"
						items="${reply}">
						<c:choose>
							...
							<c:otherwise>
								...
								<c:if test="${replyList.u_no eq user.u_no}">
									<button type="button" class="deleteBtn"
										data-reply-id="${replyList.r_no}">삭제</button>
								</c:if>
								<br>
								내용: ${replyList.r_text}								
								<hr>
				...
				${replyPaging.goReplyToLastPage()} <br> <input type="text"
					style="width: 200px" id="replyText" name="replyText">
